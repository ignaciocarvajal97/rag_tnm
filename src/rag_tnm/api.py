...
def create_app() -> FastAPI:
    ...
    def ingest(p: IngestPayload):
        ...
        cat = Path(p.catalog_dir) if p.catalog_dir else None
        try:
            return run_ingest(reset=p.reset, catalog_dir=cat)
    ...
    @app.post("/search")
    def search(p: QuestionPayload):
        try:
            return {"hits": run_search(p.question, k=p.k)}
        except Exception as e:
            raise HTTPException(503, str(e)) from e

    @app.post("/context")
    def context(p: QuestionPayload):
        try:
            return run_context(p.question, k=p.k)
        except Exception as e:
            raise HTTPException(503, str(e)) from e
